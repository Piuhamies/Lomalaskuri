...
export class DefaultMenu extends React.Component {
  ...
  componentDidMount() {
    let curSchoolUrl = window.location.pathname.substring(
      1,
      window.location.pathname.indexOf("/", 1)
    );
    let curSchoolElem =
      this.props.schools[
        this.props.schools.findIndex((i) => i.href === curSchoolUrl)
      ];
    var properties =
      curSchoolElem != null && curSchoolElem !== "undefined"
        ? curSchoolElem.theme
        : this.props.schools[0].theme;
    this.props.updateDarkMode(properties, true);
    if (this.state.reset === true && this.state.prevReset === false) {
      this.setState({
        curSchool: { curSchoolUrl: curSchoolUrl, curSchoolElem: curSchoolElem },
        reset: true,
        prevReset: true,
      });
  ...
  toggle = () => {
    ...
    var properties =
      this.state.curSchool.curSchoolElem != null &&
      this.state.curSchool.curSchoolElem !== "undefined"
        ? this.state.curSchool.curSchoolElem.theme
        : this.props.schools[0].theme;
  ...
  render() {
    let curSchool =
      this.state.curSchool !== null
        ? this.state.curSchool.curSchoolElem
