...
function MainLoop() {
    ...
    timers.forEach(function(element, index) {
        ...
        for(len = timeNames.length, i=0; i<len; ++i) { //kunnolla optimoitu for-looppi prefix lisäyksellä ja kaikella 
            ...
            for(b= slider; b<timeNames.length; ++b) { //jos slider arvo ei yletä tähän kohtaan niin
                btnElem[b-1].classList.remove("check");
                timers[index].elem[b].style.display = "none";
            }
            for(a = 1; a<slider; a++) {
                btnElem[a-1].classList.add("check");
            }   
      }
    }
    else {
...
function LatestWeekend(nyt) {
    var otsikko = document.getElementById("glimpseOtsikko");
    if(timers[0].start-nyt > 0) {
        var start = timers[0].start.getTime();
        otsikko.textContent = timers[0].nimi +":";
    }
    else if(timers[0].end-nyt > 0){
        otsikko.textContent = "Aikaa loman loppuun 😢:";
        var start = timers[0].end.getTime();
    }
    let distance = [
        Math.floor((start - nyt) / (1000* 60 * 60* 24* 7)), //maaginen seiskalla jako(viikot)
        Math.floor((start - nyt) % (1000 * 60 *60*24 *7) / (1000* 60* 60 *24)), //päivät
    ...
    distance.forEach((element, index) => {
        var tempElem = document.getElementById("glimpse" + timeNames[index].shortened);
        tempElem.textContent = distance[index] + timeNames[index].shortened;
    });
}
