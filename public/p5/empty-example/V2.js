...
function randomizeMatoLocation() {
  matoCoords.push([
    int(random(0, gridX - matoStartingLength)),
    int(random(0, gridY)),
  ]);
  for (var i = 0; i < matoStartingLength - 1; ++i) {
    //looppi joka arvoo randomilla wormin sijainnin
...
function detectPizzaCollision() {
  ...
  if (
    lastX > pizzaX &&
    lastY > pizzaY &&
    lastX < pizzaX + (pizzaSize / 2) * skipX &&
    lastY < pizzaY + (pizzaSize / 2) * skipY
  ) {
    getNewPizzaLocation();
    ++score;
    for (var i = 0; i < matoGrow; ++i) {
      //looppi joka arvoo randomilla wormin sijainnin
...
function startGame() {
  ...
  main = setInterval(function () {
    ...
    for (len = matoCoords.length, i = 0; i < len; ++i) {
      ...
      if (i < len - 1) {
        line(
          matoCoords[i][0] * skipX,
          matoCoords[i][1] * skipY,
          matoCoords[i + 1][0] * skipX,
          matoCoords[i + 1][1] * skipY
        );
      }
    }
  }, updateInterval);
