...
export default class TimerClass extends React.Component {
  ...
  createTimerElems() {
    ...
    timers.forEach(function (element, index) {
      var oldIndex = index;
      var div = (
        <div key={index + "timerDivKey"} id={"timer" + index}>
          <h2 className="alaotsikot">{element.nimi}</h2>
          <div className="ajat">
            {timeNames.map((x, index) => (
              <p
                id={x.nimi + oldIndex}
      ...
      timers.forEach(function (element, index) {
        ...
        if (start - nyt > 0) {
          ...
          let distance = [
            ...
            Math.floor(
              ((start - nyt) % (1000 * 60 * 60 * 24 * 7)) /
                (1000 * 60 * 60 * 24)
            ), //päivät
            Math.floor(
              ((start - nyt) % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60)
            ), // tunnit
          ...
          for (let i = 0; i < timeNames.length; ++i) {
            ...
            for (let b = slider; b < timeNames.length; ++b) {
          ...
          let distance = [
            ...
            Math.floor(
              ((end - nyt) % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60)
            ), // tunnit
  ...
  render() {
    return (
      <>
        ...
        <div id="timerArea">{this.createTimerElems()}</div>
      </>
    );
  }
}
